...
def normalize_tile_parallel(row) -> Dict:
    """
    ...
    # Check if we're getting a tuple directly or a value from dask apply
    #if isinstance(tile_info, (tuple, list)):
    #    input_path, output_path, means, stds = tile_info
    #else:
        # When coming from dask apply, tile_info will be the value itself
    #    input_path, output_path, means, stds = tile_info[0]  # Unpack the first element which should be our tuple

    input_path = row['input_path']
    try:

        # Extract the tuple components from the row
        #input_path = row['input_path']
        output_path = row['output_path']
        means = row['means']
        stds = row['stds']

        # Check if means/stds are strings and convert if necessary
        ...
        with rasterio.open(input_path) as src:
            ...
            for band in range(data.shape[0]):
                ...
                try:
                    ...
                    raise TypeError(f"Type mismatch in normalization: data:{data[band].dtype}, "
                                  f"means:{means.dtype}, stds:{stds.dtype}") from e
    ...
    try:
        ...
        with setup_dask_cluster(n_workers=n_workers, logger=logger) as client:
            ...
            df = pd.DataFrame({
                ...
                'means': [means.copy() for _ in range(len(tile_paths))], 
