...
if __name__ == '__main__':
    ...
    workstations = dask.distributed.SSHCluster(
        hosts = ['127.0.0.1','10.8.3.184'],
        connect_options = {"known_hosts": None, 'username': "diegobengochea", 'password' : '1234'},
    )

    # Start dask cluster
    client = dask.distributed.Client(workstations)
    ...
    args_list = list(itertools.product(geotiles_spain, [2017]))
    ...
    for args in args_list:
        ...
        target_dataset.rio.to_raster(f'/Users/diegobengochea/git/iberian.carbon/data/Sentinel2_Composites_Spain/sentinel2_mosaic_{year}_lat{bbox[3]}_lon{bbox[0]}_10m.tif')
