...
##############################################################################################################
# Metrics ignoring heights lower than 1 meter to avoid counting shrubs and artificial areas

def _nan_robust_thresholded_mean_absolute_error_update(preds: Tensor, target: Tensor, nan_value: float, height_threshold: float) -> Tuple[Tensor, int]:
    """Update and returns variables required to compute Mean Absolute Error removing NaN values before reduction.
    ...
    abs_error = torch.masked_select(abs_error,target_mask) 
    
    sum_abs_error = torch.sum(abs_error)
    return sum_abs_error, target.shape[0]

def _nan_robust_thresholded_mean_absolute_percentage_error_update(
    preds: Tensor,
    target: Tensor,
    nan_value: float,
    ...
    target_mask = (target!=nan_value) & (target>height_threshold)
    abs_per_error = torch.masked_select(abs_per_error,target_mask) 
...
def _nan_robust_thresholded_mean_squared_error_update(preds: Tensor, target: Tensor, nan_value: float, height_threshold: float) -> Tuple[Tensor, int]:
...
class NanRobustThresholdedMeanAbsoluteError(Metric):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        sum_abs_error, num_obs = _nan_robust_thresholded_mean_absolute_error_update(preds, target, nan_value=self.nan_value, height_threshold = self.height_threshold)
...
class NanRobustThresholdedMeanAbsolutePercentageError(Metric):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        """Update state with predictions and targets."""
        sum_abs_per_error, num_obs = _nan_robust_thresholded_mean_absolute_percentage_error_update(preds, target, self.nan_value, height_threshold = self.height_threshold)
...
class NanRobustThresholdedMeanSquaredError(Metric):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        """Update state with predictions and targets."""
        sum_squared_error, num_obs = _nan_robust_thresholded_mean_squared_error_update(preds, target, nan_value = self.nan_value, height_threshold = self.height_threshold)
